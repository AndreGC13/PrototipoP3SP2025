...
import Modelo.seguridad.FacultadesDAO;
...
public class MantenimientoFacultades extends javax.swing.JInternalFrame {
  ...
  // Método para llenar la tabla de usuarios
    public void llenadoDeTablas() {
        ...
        modelo.addColumn("Contraseña Facultades");

        FacultadesDAO usuarioDAO = new FacultadesDAO();
        List<Facultades> usuarios = usuarioDAO.select();

        tablaFacultades.setModel(modelo);
        String[] dato = new String[3];
        for (int i = 0; i < usuarios.size(); i++) {
            dato[0] = Integer.toString(usuarios.get(i).getcodigo_facultad());
            dato[1] = usuarios.get(i).getnombre_facultad();
            // No mostrar la contraseña en texto claro, sino "********"
            dato[2] = "********";
            modelo.addRow(dato);
        }
    }
public void registrarFacultades() {
    try {
        ...
        // Crear el objeto Facultades
        Facultades nuevoFacultades = new Facultades();
        nuevoFacultades.setnombre_facultad(username);
        nuevoFacultades.setestatus_facultad(contrasenaEncriptada);

        // Insertar el nuevo usuario en la base de datos
        FacultadesDAO usuarioDAO = new FacultadesDAO();
        usuarioDAO.insert(nuevoFacultades);

        JOptionPane.showMessageDialog(null, "Facultades registrado con éxito.");
    ...
    public void buscarFacultades() {
        try {
            ...
            int idFacultades = Integer.parseInt((String) txtbuscado.getText());

            // Crear el objeto usuario a consultar
            Facultades usuarioAconsultar = new Facultades();
            usuarioAconsultar.setcodigo_facultad(idFacultades);
            ...
            if (usuarioAconsultar != null) {
                ...
                if (usuarioAconsultar.getestatus_facultad().equals(contrasenaEncriptada)) {
                    // Si las contraseñas coinciden, mostrar la información
                    txtNombre.setText(usuarioAconsultar.getnombre_facultad());
                    txtPassword.setText(usuarioAconsultar.getestatus_facultad());
                } else {
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        FacultadesDAO usuarioDAO = new FacultadesDAO();
        Facultades usuarioAEliminar = new Facultades();
        usuarioAEliminar.setcodigo_facultad(Integer.parseInt(txtbuscado.getText()));
        usuarioDAO.delete(usuarioAEliminar);
        llenadoDeTablas();
        UsuarioConectado usuarioEnSesion = new UsuarioConectado();
        int resultadoBitacora=0;
        Bitacora bitacoraRegistro = new Bitacora();
        resultadoBitacora = bitacoraRegistro.setIngresarBitacora(usuarioEnSesion.getIdUsuario(), APLICACION100,  "Borrar Datos Facultadess");
    ...
    try {
        ...
        Facultades usuarioAInsertar = new Facultades();
        
        // Establecer el nombre de usuario
        usuarioAInsertar.setnombre_facultad(txtNombre.getText());
        ...
        usuarioAInsertar.setestatus_facultad(contrasenaEncriptada);
...
//        // TODO add your handling code here:
        FacultadesDAO usuarioDAO = new FacultadesDAO();
        Facultades usuarioAActualizar = new Facultades();
        usuarioAActualizar.setcodigo_facultad(Integer.parseInt(txtbuscado.getText()));
        usuarioAActualizar.setnombre_facultad(txtNombre.getText());
        usuarioAActualizar.setestatus_facultad(txtPassword.getText());
        ...
        int resultadoBitacora=0;
        Bitacora bitacoraRegistro = new Bitacora();
        resultadoBitacora = bitacoraRegistro.setIngresarBitacora(usuarioEnSesion.getIdUsuario(), APLICACION100,  "Actualizacion Datos Facultadess");
